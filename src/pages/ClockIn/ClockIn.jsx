...
const CheckIn = () => {
  ...
  const handleClockIn = async () => {
    if (
      ...
      showToast("Please select Office/Client location.", "error");

      if (Clientdata.length > 1 && !selectedClientId && !isWorkFromOffice) {
        setOpenClientSelectModal(false);
        return;
      }
      if (Locationdata.length > 1 && !selectedLocationId && isWorkFromOffice) {
